#!/usr/bin/env python3
"""Validate one MLB divisions response and build transient RML context.
...
ENDPOINT_KINDS = {"divisions": "division"}
...
def build_context(
    ...
    *,
    ...
    if endpoint_family != "divisions":
        raise ContractError("the mlb-divisions connector accepts only the divisions endpoint")
    raw_records = payload.get("divisions")
    ...
    for index, raw_record in enumerate(raw_records):
        path = f"$.divisions[{index}]"
        record = _object(raw_record, path)
        add_organization("division", record, path, required=True)
        _validate_sport(record, path)
        _season_code(record.get("season"), f"{path}.season")
        add_organization("league", record.get("league"), f"{path}.league", required=False)
    ...
    return {
        "_baseballO": {
            ...
            "sourceModule": "mlb-divisions",
            "inputValidation": "passed",
            "counts": {"organizations": len(organization_rows)},
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate transient MLB divisions JSON and build RML context."
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"mlb-divisions input rejected: {error}", file=sys.stderr)
        raise SystemExit(2)
