#!/usr/bin/env python3
"""Validate one MLB teams response and build transient RML context.
...
def build_context(
    ...
    *,
    ...
    if endpoint_family != "teams":
        raise ContractError("the mlb-teams connector accepts only the teams endpoint")
    raw_records = payload.get("teams")
    ...
    for index, raw_record in enumerate(raw_records):
        ...
        add_organization("team", record, path, required=True)
        ...
        add_organization(
            "division", record.get("division"), f"{path}.division", required=False
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate transient MLB teams JSON and build RML context."
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"mlb-teams input rejected: {error}", file=sys.stderr)
        raise SystemExit(2)
