...
const FAMILY_DEFINITIONS = {
  ...
  pitching: {
    ...
    corePatterns: [
      ...
      `BIND(
        IF(EXISTS {
          ?pitchResultRecord a base:BaseballEventRecord ; cco:ont00001808 ?pitch, ?ballProcess .
          ?ballProcess a base:BallProcess .
        }, "B",
        IF(EXISTS {
          ?pitchResultRecord a base:BaseballEventRecord ; cco:ont00001808 ?pitch, ?foulProcess .
          ?foulProcess a ?foulClass .
          VALUES ?foulClass { base:FoulBallProcess base:FoulTipProcess }
        }, "F",
        IF(EXISTS {
          ?pitchResultRecord a base:BaseballEventRecord ; cco:ont00001808 ?pitch, ?fairProcess .
          ?fairProcess a base:FairBallProcess .
        }, "X",
        IF(EXISTS {
          ?hitByPitch a base:HitByPitchProcess ; obo:BFO_0000132 ?plateAppearance .
          ?pitch obo:BFO_0000199/obo:BFO_0000224 ?pitchEndInstant .
          ?pitchEndTimestamp a base:BaseballTimestampICE ;
              cco:ont00001916 ?pitchEndInstant ; cco:ont00001767 ?pitchEnd .
          FILTER NOT EXISTS {
            ...
            ?laterPitchEndTimestamp a base:BaseballTimestampICE ;
                cco:ont00001916 ?laterPitchEndInstant ; cco:ont00001767 ?laterPitchEnd .
            FILTER (?laterPitchEnd > ?pitchEnd)
          }
        }, "H",
        IF(EXISTS {
          ?pitchResultRecord a base:BaseballEventRecord ; cco:ont00001808 ?pitch, ?calledStrike .
          ?calledStrike a base:StrikeCallAct .
        }, "C",
        IF(EXISTS {
          ?pitchResultRecord a base:BaseballEventRecord ; cco:ont00001808 ?pitch, ?strikeProcess, ?battingAct .
          ?strikeProcess a base:StrikeProcess .
          ?battingAct a ?battingActClass .
          VALUES ?battingActClass { base:SwingAct base:BuntAct }
        }, "S", "U")))))) AS ?pitchCallCode)`,
    ],
    patternComponents: {
      fielding_team: [
        `?halfInning a base:HalfInning ;
    ...
    metrics: {
      ...
      balls: {
        ...
        select: '(SUM(IF(?pitchCallCode = "B", 1, 0)) AS ?balls)',
      ...
      swinging_strikes: {
        ...
        select: '(SUM(IF(?pitchCallCode = "S", 1, 0)) AS ?swingingStrikes)',
      ...
      fouls: {
        ...
        select: '(SUM(IF(?pitchCallCode = "F", 1, 0)) AS ?fouls)',
      ...
      in_play: {
        ...
        select: '(SUM(IF(?pitchCallCode = "X", 1, 0)) AS ?inPlay)',
  ...
  baserunning: {
    ...
    corePatterns: [
      ...
      `OPTIONAL {
        ?runnerRecord cco:ont00001808 ?specificResolution .
...
function renderFilter(dimension, value, id) {
  ...
  } else if (dimension.input === "event-category") {
    for (const item of values) {
      if (typeof item !== "string" || !/^[a-z0-9_]+$/u.test(item)) {
        throw new TypeError(`${id} values must be lowercase event categories`);
      }
    }
  } else {
