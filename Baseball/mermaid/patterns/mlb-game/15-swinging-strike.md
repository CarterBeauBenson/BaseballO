<!-- Generated by scripts/generate_rml_mermaid.py; do not edit. -->
<!-- Mapping SHA-256: b89377d634df094aef78de708e811ac44b66284dbbc0ba86bb21cde578077127 -->

# Swinging strike without contact - MLB game RML

A neutral swing act precedes the counted strike process; no distinct successful or unsuccessful swing class is minted.

Solid relation arrows are explicit parent-triples-map joins. Dotted arrows match IRI templates or IRI-valued references within the same logical source. Cross-pattern targets are listed in the table rather than drawn. Unresolved dynamic resource types remain generic; the accepted design supplies their reviewed interpretation.

```mermaid
flowchart LR
    S01["Source: MissedStrikeSource"]
    S02["Source: SwingingStrikeSource"]
    M01["SwingingStrikeSwingActMap<br/>Swing Act"]
    M02["SwingingStrikeSwingActMapRecordAboutMap<br/>relationship overlay"]
    M03["SwingingStrikeProcessMap<br/>Strike Process"]
    M04["SwingingStrikeProcessMapRecordAboutMap<br/>relationship overlay"]
    S02 --> M01
    S02 --> M02
    S01 --> M03
    S01 --> M04
    M02 -.->|is about; template| M01
    M03 -.->|preceded by; template| M01
    M04 -.->|is about; template| M03
```

## Logical sources

| Source | Execution file | JSONPath iterator | Maps in this review |
| --- | --- | --- | ---: |
| `MissedStrikeSource` | `game-context.json` | `$.liveData.plays.allPlays[*].playEvents[?(@.isPitch == true && @.details.call.code == 'S' \|\| @.isPitch == true && @.details.call.code == 'W' \|\| @.isPitch == true && @.details.call.code == 'M')]` | 2 |
| `SwingingStrikeSource` | `game-context.json` | `$.liveData.plays.allPlays[*].playEvents[?(@.isPitch == true && @._baseballO.isBuntAttempt == false && @.details.call.code == 'S' \|\| @.isPitch == true && @._baseballO.isBuntAttempt == false && @.details.call.code == 'W')]` | 2 |

## Triples maps

| Triples map | Subject class | Emitted relations | Cross-pattern targets |
| --- | --- | --- | --- |
| `SwingingStrikeSwingActMap` | Swing Act | has participant, occurrent part of, occurs at, preceded by, realizes | has participant -> Player (template), has participant -> SwingBaseballBatMap (template), occurrent part of -> Plate Appearance (template), occurrent part of -> Referenced resource (IRI reference), occurs at -> Baseball Field (template), preceded by -> PitchActMap (template), realizes -> Batter (template) |
| `SwingingStrikeSwingActMapRecordAboutMap` | relationship overlay | is about | none |
| `SwingingStrikeProcessMap` | Strike Process | occurrent part of, occurs at, preceded by | occurrent part of -> Plate Appearance (template), occurs at -> Baseball Field (template) |
| `SwingingStrikeProcessMapRecordAboutMap` | relationship overlay | is about | none |
