...
flowchart LR
    ...
    PLAYERROLE["personId + teamId<br/>PlayerRole"] --> GAME
    SPECIALROLE["personId + role type<br/>Batter/Pitcher/etc."] --> ACT
```
...
Runner objects receive execution-only enclosing `atBatIndex` and zero-based
`runnerIndex` values in the disposable context. Those values establish
collision-safe record, act, and resolution identities and link supported runner
acts and results to the correct plate appearance without rewriting the source.
