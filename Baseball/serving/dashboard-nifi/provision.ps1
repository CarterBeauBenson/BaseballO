...
if (
    [string]$contract.artifactType -ne 'baseballo-nifi-dashboard-sql-flow-contract' -or
    ...
    throw "Unsupported Dashboard SQL NiFi contract: $contractPath"
}
...
foreach ($required in @($materializer, $schema)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Dashboard SQL artifact is missing: $required"
    }
}
...
function Get-OrCreateProcessGroup([string] $ParentId, [string] $Name, [int] $X, [int] $Y) {
    ...
    return (Invoke-NiFi -Method POST -Path "/process-groups/$ParentId/process-groups" -Body @{
        ...
        component = @{
            name = $Name
            position = @{ x = $X; y = $Y }
            comments = 'Incremental dashboard SQL from existing promoted RDF; game checkpoints and independent publication.'
        }
    }).id
}
...
function Stop-Group([string] $GroupId) {
    ...
    throw "Dashboard SQL did not stop for reconciliation."
}
...
function Ensure-Processor {
    ...
    if ($matches.Count -gt 1) { throw "More than one processor is named '$Name' in Dashboard SQL." }
...
$groupId = Get-OrCreateProcessGroup $baseballId ([string]$contract.processGroup) 950 1500
$active = @((Get-GroupFlow $groupId).processors | Where-Object { [int]$_.status.aggregateSnapshot.activeThreadCount -gt 0 })
if ($active.Count -gt 0) { throw 'Dashboard worker is active; leave its checkpoints and process untouched.' }
Stop-Group $groupId
...
$processors.materialize = Ensure-Processor $groupId 'Materialize Dashboard Changes' 'org.apache.nifi.processors.standard.ExecuteStreamCommand' @{
    ...
    'Argument Delimiter' = ';'
    'Ignore STDIN' = 'true'
    'Output Destination Attribute' = 'dashboard.materialization.output'
    'Max Attribute Length' = '65536'
    'Output MIME Type' = 'application/json'
} @('output stream', 'nonzero status') 300 0
$processors.gate = Ensure-Processor $groupId 'Require Dashboard Materialization Success' 'org.apache.nifi.processors.standard.RouteOnAttribute' @{
    'Routing Strategy' = 'Route to Property name'; 'passed' = "`${execution.status:equals('0')}"
} @() 620 0
$processors.retry = Ensure-Processor $groupId 'Retry Dashboard Materialization' 'org.apache.nifi.processors.standard.RetryFlowFile' @{
    'Retry Attribute' = 'retry.dashboard-materialization'
...
$processors.success = Ensure-Processor $groupId 'Record Dashboard Materialization Result' 'org.apache.nifi.processors.standard.LogAttribute' @{
    'Log Level' = 'info'; 'Log Payload' = 'false'; 'Attributes to Log Regular Expression' = '^(dashboard|execution)\..*$'
    'Log FlowFile Properties' = 'true'; 'Output Format' = 'Line per Attribute'
    'Log Prefix' = 'BaseballO Dashboard SQL materialization'; 'Character Set' = 'UTF-8'
} @('success') 940 -80
$processors.failure = Ensure-Processor $groupId 'Record Dashboard Materialization Failure' 'org.apache.nifi.processors.standard.LogAttribute' @{
    'Log Level' = 'error'; 'Log Payload' = 'false'; 'Attributes to Log Regular Expression' = '^(dashboard|execution|retry)\..*$'
    'Log FlowFile Properties' = 'true'; 'Output Format' = 'Line per Attribute'
    'Log Prefix' = 'BaseballO Dashboard SQL materialization failure'; 'Character Set' = 'UTF-8'
} @('success') 940 250

Ensure-Connection $groupId 'dashboard update trigger' $processors.trigger $processors.materialize @('success')
...
$expected = @(
    'Dashboard Update Check', 'Materialize Dashboard Changes', 'Require Dashboard Materialization Success',
    'Retry Dashboard Materialization', 'Record Dashboard Materialization Result', 'Record Dashboard Materialization Failure'
)
$unexpected = @((Get-GroupFlow $groupId).processors | Where-Object { $_.component.name -notin $expected })
if ($unexpected.Count -gt 0) {
    throw "Dashboard SQL contains unexpected processors: $(@($unexpected.component.name) -join ', ')"
}
$invalid = @()
...
if ($invalid.Count -gt 0) { throw "Dashboard SQL has invalid processors:`n$($invalid -join "`n")" }

if ($Start) {
    # Downstream processors first, then the bounded periodic trigger. One
    # waiting tick is enough while the single writer is occupied.
    $ordered = @((Get-GroupFlow $groupId).processors | Sort-Object { if ($_.id -eq $processors.trigger) { 1 } else { 0 } })
    ...
    Write-Host "Enabled independent Dashboard SQL group $groupId. NiFi owns asynchronous updates."
}
else { Write-Host "Provisioned Dashboard SQL group $groupId in STOPPED state." }
