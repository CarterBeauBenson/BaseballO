...
## Plate Appearance Quality decision support

`plate-appearance-fingerprint.rq` deliberately returns its evidence components
rather than asserting a permanent classification. PAQ-1.0 assigns an outcome
rating, a bounded version of the existing
...
`whiff-and-take-profiles.rq` now resolves swing, contact, ball, and called-
strike evidence once per scoped pitch before evaluating absence. This fixes the
former correlated-optional undercount of takes and reduces the eight-game plan
from Fuseki's timeout boundary to well under one second.
