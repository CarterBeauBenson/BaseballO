...
| `10-game-dimensions.rq` | `idx:GameFact` | Game, mapped start timestamp, field/venue chain, containing Season Phase, and containing Season |
...
| `40-pitches.rq` | `idx:PitchFact` | Pitch act, CCO Person pitcher, following pitch motion, plate appearance/game, field/venue, and any current reviewed nominal pitch classification |
| `50-pitch-calls.rq` | `idx:PitchCallFact` | Complete pitch pattern and the specific supported ball, called-strike, swinging-strike, foul, in-play, or hit-by-pitch evidence |
...
The test suite compares exact distinct row sets for the semantic families in
both directions, including current pitch/trajectory classifications and a
...
nineteen measured routes. Auto mode selects the index for sixteen proven pairs
and retains three reviewed routes on the authoritative layer. Before indexed execution,
...
Nineteen indexed companions under [`benchmarks/indexed/`](benchmarks/indexed/) span
...
The preserved eight-game corpus capture shows median improvements for sixteen
reviewed pairs (1.509x to 1923.986x). The admitted `hits-by-season` route
improves by 799.406x and reduces its direct TDB2 plan from 15 quad patterns to
4. Direct TDB2 execution traces are stored beside the timing baselines.
...
decisions, records, roles, or temporal structure. For archived/manual inputs,
rehydration can reload or rematerialize the authoritative graph from preserved
raw JSON plus the recorded RML, context-builder, mapper, and hash provenance.
For transient API inputs, the persistent authoritative RDF is the durable
research record. Both paths regenerate the disposable index; neither treats
the index as authoritative.

The archived-input portable package and exact-restore contract is documented in
