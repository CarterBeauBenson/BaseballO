...
Package version 1 is a portable, closed inventory for one completed game whose
source JSON was intentionally archived, such as a manual input or checked-in
fixture. It
...
| --- | --- |
| `raw/` | Byte-identical archived/manual completed-game JSON for this package variant |
...
Scheduled API payloads are transient and cannot satisfy package version 1 after
promotion cleanup. Their durable recovery boundary is the persistent
authoritative RDF plus provenance/contracts; that RDF regenerates the indexed
graph and SQL serving rows without claiming the deleted API response is still
available.
