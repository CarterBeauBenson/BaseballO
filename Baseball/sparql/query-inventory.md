...
This inventory records what each of the 51 canned queries counts or discovers
...
The 51 canned queries remain authoritative-pattern queries. The separate
...
runner routes sixteen proven companions to the index while preserving the
...
[`advanced/`](advanced/). That 17-query suite does not change this inventory's
51-query count, and none of its queries is silently treated as index-backed.
...
49 queries are structurally index-ready, while `empty-games-prototype.rq` and
...
| Root | `empty-games-prototype.rq` | Candidate player/game pair | Batter participation plus reviewed contribution patterns; reviewed world-side result-class completeness gate | Player, game | Aligned prototype |
...
| Pitching | `pitch-types-by-pitcher.rq` | Distinct `PitchAct` | Reviewed authoritative nominal classification evidence | Pitcher, pitch type | Aligned |
...
| Pitching | `batted-ball-trajectories.rq` | Distinct batted-ball motion | Reviewed authoritative nominal trajectory evidence | Batter, trajectory | Aligned |
| Baserunning | `events-by-player.rq` | Distinct runner resolution | Runner participant + reviewed world-side resolution/event class supplies stable event category | Player, event type | Aligned |
...
| Games | `games-by-season-phase.rq` | Distinct game | Game containment in an accepted season-phase process | Season, phase | Aligned |
...
| Options | `available-baserunning-events.rq` | Distinct stable event category | Runner resolution + reviewed world-side resolution/event class | Event type | Aligned |
