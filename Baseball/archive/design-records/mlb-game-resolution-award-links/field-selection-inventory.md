...
| `P.runners[].movement.start` | Already supplied | Reported movement start | Base token alone does not provide reviewed consequence-boundary state. Missing is not automatically HOME. |
...
The [resolution-link proposal](resolution-links-review.md) uses the same
