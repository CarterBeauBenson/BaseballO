...
if (-not (Test-TcpPort -HostName '127.0.0.1' -Port 3030)) {
    throw 'Fuseki is not listening on port 3030.'
}
$askResponse = Invoke-RestMethod -Uri 'http://127.0.0.1:3030/baseball-dev/query' -Method Post -Body @{ query = 'ASK { }' } -Headers @{ Accept = 'application/sparql-results+json' }
...
if ($WriteProbe) {
    ...
    $dataEndpoint = "http://127.0.0.1:3030/baseball-dev/data?graph=$graphParameter"
    ...
    try {
        ...
        $probeResponse = Invoke-RestMethod -Uri 'http://127.0.0.1:3030/baseball-dev/query' -Method Post -Body @{ query = $query } -Headers @{ Accept = 'application/sparql-results+json' }
...
if (-not (Test-TcpPort -HostName '127.0.0.1' -Port 8443)) {
    throw 'NiFi is not listening on port 8443.'
}
$curl = Get-Command curl.exe -ErrorAction SilentlyContinue
if ($null -ne $curl) {
    $status = (& $curl.Source --silent --insecure --output NUL --write-out '%{http_code}' 'https://127.0.0.1:8443/nifi/').Trim()
