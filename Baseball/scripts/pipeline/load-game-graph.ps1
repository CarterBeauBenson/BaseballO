...
try {
    ...
    $dataEndpoint = "http://127.0.0.1:3031/baseball-dev/data?graph=$encodedGraph"
    ...
    $askResult = Invoke-RestMethod -Uri 'http://127.0.0.1:3031/baseball-dev/query' -Method Post -Body @{ query = $ask } -Headers @{ Accept = 'application/sparql-results+json' }
