...
EXPLORE_FAMILIES = {
    "batting": {
        ...
        "metrics": {
            ...
            "hits": ("hits", "COUNT(DISTINCT CASE WHEN f.event_type IN ('single','double','triple','home_run') THEN f.result_iri END)"),
            "singles": ("singles", "COUNT(DISTINCT CASE WHEN f.event_type='single' THEN f.result_iri END)"),
            "doubles": ("doubles", "COUNT(DISTINCT CASE WHEN f.event_type='double' THEN f.result_iri END)"),
            "triples": ("triples", "COUNT(DISTINCT CASE WHEN f.event_type='triple' THEN f.result_iri END)"),
            "home_runs": ("homeRuns", "COUNT(DISTINCT CASE WHEN f.event_type='home_run' THEN f.result_iri END)"),
            "walks": ("walks", "COUNT(DISTINCT CASE WHEN f.event_type='walk' THEN f.result_iri END)"),
            "strikeouts": ("strikeouts", "COUNT(DISTINCT CASE WHEN f.event_type='strikeout' THEN f.result_iri END)"),
            "total_bases": ("totalBases", "COUNT(DISTINCT CASE WHEN f.event_type='single' THEN f.result_iri END) + 2*COUNT(DISTINCT CASE WHEN f.event_type='double' THEN f.result_iri END) + 3*COUNT(DISTINCT CASE WHEN f.event_type='triple' THEN f.result_iri END) + 4*COUNT(DISTINCT CASE WHEN f.event_type='home_run' THEN f.result_iri END)"),
