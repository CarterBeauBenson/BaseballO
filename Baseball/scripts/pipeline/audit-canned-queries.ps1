...
if ($queryFiles.Count -ne 48) {
    throw "Expected 48 authoritative canned queries; found $($queryFiles.Count)."
}
...
foreach ($queryFile in $queryFiles) {
    ...
    $results += [ordered]@{
        query = $relativePath
        family = $family
        querySha256 = (Get-FileHash -LiteralPath $queryFile.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
