...
foreach ($entry in $catalogQueries) {
    ...
    $expectedGraphVariables = @('?graph')
    if (($graphVariables -join '|') -ne ($expectedGraphVariables -join '|')) {
        throw "$($entry.path) uses an unexpected named-graph variable set: $($graphVariables -join ', ')."
    }
...
$report = [ordered]@{
    ...
    catalogSha256 = Get-CanonicalTextSha256 -Path $catalogPath
...
if ($VerifyBaseline) {
    ...
    $capture = @($register.captures | Where-Object { [string]$_.id -eq 'advanced-2026-08-03' })
    if ($capture.Count -ne 1) { throw 'Advanced-query evidence register entry is missing or ambiguous.' }
    ...
    ) { throw 'Current advanced-query text differs from the reviewed historical-capture compatibility set.' }
