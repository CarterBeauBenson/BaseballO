...
foreach ($gamePk in $gamePks) {
    ...
    $contractHashes += [string]$manifest.contractSha256
}
$contractHashes = @($contractHashes | Sort-Object -Unique)
if ($contractHashes.Count -ne 1) {
    throw "The corpus contains multiple query-index contract hashes: $($contractHashes -join ', ')"
}
...
foreach ($pair in @($pairManifest.pairs)) {
    ...
    $benchmarkResults += [ordered]@{
        ...
        authoritativeQuerySha256 = (Get-FileHash -LiteralPath $authoritativeFile -Algorithm SHA256).Hash.ToLowerInvariant()
        indexedQuery = $indexedPath
        indexedQuerySha256 = (Get-FileHash -LiteralPath $indexedFile -Algorithm SHA256).Hash.ToLowerInvariant()
...
$report = [ordered]@{
    ...
    queryIndexContractSha256 = [string]$contractHashes[0]
...
$markdown.Add("- Query-index contract SHA-256: ``$($contractHashes[0])``")
$markdown.Add('')
