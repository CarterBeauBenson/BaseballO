...
if (Test-TcpPort -HostName '127.0.0.1' -Port 3031) {
...
try {
    ...
    foreach ($pair in @($pairManifest.pairs)) {
        foreach ($layer in @('authoritative', 'indexed')) {
            ...
            $results += [ordered]@{
                ...
                layer = $layer
                query = $relativePath
                querySha256 = Get-CanonicalTextSha256 -Path $queryFile
                log = $logName
    ...
    $queryIndexSemanticAdmission = Get-QueryIndexSemanticAdmission
    $report = [ordered]@{
        ...
        queryIndexSemanticContractId = [string]$queryIndexSemanticAdmission.ContractId
        queryIndexSemanticContractSha256 = [string]$queryIndexSemanticAdmission.ContractSha256
        queryIndexImplementationSha256 = Get-QueryIndexImplementationHash
    ...
    $markdown.Add("- Query-index semantic contract: ``$($report.queryIndexSemanticContractId)`` (``$($report.queryIndexSemanticContractSha256)``)")
    $markdown.Add("- Query-index implementation SHA-256 (provenance only): ``$($report.queryIndexImplementationSha256)``")
    $markdown.Add('')
