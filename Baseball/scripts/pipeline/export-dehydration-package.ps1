...
$indexAdmission = Resolve-QueryIndexManifestAdmission -Manifest $indexManifest
...
$repositoryContractFiles = @(
    (Join-Path $script:RepositoryRoot 'sources\mlb-game\mapping\mlb-game.rml.ttl'),
    (Join-Path $script:RepositoryRoot 'sources\mlb-game\mapping\validate_mlb_game_mapping.py'),
    ...
    (Join-Path $script:RepositoryRoot 'scripts\pipeline\validate-shacl.py'),
    (Join-Path $script:RepositoryRoot 'sources\mlb-game\shacl\authoritative.ttl'),
    (Join-Path $script:RepositoryRoot 'shacl\query-index.ttl'),
    (Join-Path $script:RepositoryRoot 'sparql\query-index\semantic-contract.json'),
    (Join-Path $script:RepositoryRoot 'sparql\query-index\operational-query-routing.json'),
...
) + @(Get-QueryIndexImplementationFiles | ForEach-Object { $_.FullName })
...
$manifest = [ordered]@{
    ...
    queryIndexGraph = [ordered]@{
        ...
        semanticContractId = [string]$indexAdmission.SemanticContractId
        semanticContractSha256 = [string]$indexAdmission.SemanticContractSha256
        implementationSha256 = [string]$indexAdmission.ImplementationSha256
        implementationFingerprintRole = 'provenance-only'
        admissionMode = [string]$indexAdmission.Mode
    }
    generation = [ordered]@{
