...
def validate() -> tuple[int, int, int]:
    ...
    local_property_declarations = {
        value
        for value in overlay.subjects(RDF.type, OWL.ObjectProperty)
        if isinstance(value, URIRef)
    }
    ...
    unknown_subjects = axiom_subjects - declared_base_classes
    if unknown_subjects:
        raise ValueError(
            "Overlay axioms target classes not declared in BaseballO.ttl: "
    ...
    unknown_classes = (named_superclasses | fillers) - declared_classes
    if unknown_classes:
        raise ValueError(
    ...
    if unknown_properties:
        raise ValueError(
            "Overlay uses object properties not declared in the repository: "
...
def main() -> None:
    ...
    print(f"Reused object properties: {property_count}")
