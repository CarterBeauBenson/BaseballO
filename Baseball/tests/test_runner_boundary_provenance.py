"""Boundary admission remains bound to one promoted source/RDF revision."""
import json
from pathlib import Path
import tempfile
import unittest

from test_runner_boundary_admission import A


class BoundaryProvenance(unittest.TestCase):
    def fixture(self,root):
        output=root/'pipeline/evidence/mlb-game/101/run/count.json';output.parent.mkdir(parents=True)
        proof=dict(artifactType='baseballo-runner-boundary-admission',contractVersion=1,gamePk='101',
        ...
        marker.write_text(json.dumps(dict(runnerBoundaryAdmission=str(output),runnerBoundaryAdmissionSha256=A.B.sha(output.read_bytes()))))
